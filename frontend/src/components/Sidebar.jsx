...
// Government of India - Ashoka Lion Capital emblem (public domain)
const GoIEmblem = () => (
  <svg viewBox="0 0 100 120" width="38" height="46" xmlns="http://www.w3.org/2000/svg" style={{ display: "block" }}>
    {/* Ashoka Chakra (wheel) base */}
    <ellipse cx="50" cy="108" rx="28" ry="4" fill="rgba(184,151,46,0.3)" />
    ...
    {[...Array(12)].map((_, i) => {
      ...
      return (
        <line
          key={i}
          x1={50 + 1.5 * Math.cos(angle)}
          y1={92 + 1.5 * Math.sin(angle)}
          x2={50 + 5.5 * Math.cos(angle)}
          y2={92 + 5.5 * Math.sin(angle)}
          stroke={GOLD}
          strokeWidth="0.8"
        />
      );
    })}
    {/* Bull (left animal silhouette) */}
    <ellipse cx="28" cy="82" rx="9" ry="5" fill={GOLD} />
    <ellipse cx="22" cy="80" rx="5" ry="4" fill={GOLD} />
    {/* Lion (right animal silhouette) */}
    ...
    <text x="50" y="117" textAnchor="middle" fontSize="5.5" fill={GOLD} fontFamily="serif" letterSpacing="0.5">
      सत्यमेव जयते
    </text>
  </svg>
);
...
const navItems = [
  ...
  {
    to: "/dashboard/report", label: "Submit Report",
  ...
  {
    to: "/dashboard/nearby", label: "Nearby Issues",
    icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg>
  },
  {
    to: "/dashboard/donor", label: "Become Donor",
  ...
  {
    to: "/dashboard/manage-reports", label: "Manage Reports",
...
function Sidebar() {
  ...
  return (
    <aside style={{
      ...
      {/* Logo with GoI Emblem */}
      <button
        onClick={() => navigate("/dashboard")}
        style={{
          background: "none", border: "none", cursor: "pointer",
          textAlign: "left", marginBottom: "32px", padding: "0 8px",
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <GoIEmblem />
          <div>
            <div style={{ fontSize: "16px", fontWeight: "700", color: "#fff", fontFamily: "Georgia, serif", lineHeight: 1.2 }}>
              OneNation
            </div>
            <div style={{ fontSize: "9px", color: "rgba(184,151,46,0.8)", letterSpacing: "0.06em", textTransform: "uppercase", fontFamily: "sans-serif", marginTop: "2px" }}>
              Digital Governance
            </div>
          </div>
        </div>
